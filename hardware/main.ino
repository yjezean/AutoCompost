...
// --- BUTTON PINS ---
const int BUTTON_PIN = 33;            // Push button for lid control
const int STIRRER_BUTTON_PIN = 32;    // Button to control stirrer
const int RELAY_BUTTON_PIN = 22;      // Button to control relay
...
const int STIRRER_SERVO_PIN = 26;     // Servo pin for stirrer
...
const char* MQTT_CMD_STIRRER = "compost/cmd/stirrer";

// --- MQTT Status Topics (Publish) ---
const char* MQTT_STATUS_FAN = "compost/status/fan";
const char* MQTT_STATUS_LID = "compost/status/lid";
const char* MQTT_STATUS_STIRRER = "compost/status/stirrer";

// --- NTP Configuration ---
const char* ntpServer = "pool.ntp.org";
const long gmtOffset_sec = 8 * 3600;  // GMT+8 (Malaysia/Singapore)
const int daylightOffset_sec = 0;    // Daylight saving offset in seconds (not used in Malaysia)
...
int lastButtonState = HIGH; // HIGH = Not Pressed (Internal Pullup)
bool isRelayOn = false;     // Tracks if the relay/motor is currently ON
int lastRelayButtonState = HIGH; // HIGH = Not Pressed (Internal Pullup)
bool isStirrerOn = false;   // Tracks if the stirrer is currently ON
int lastStirrerButtonState = HIGH; // HIGH = Not Pressed (Internal Pullup)
unsigned long lastDHTRead = 0;  // Last time DHT was read
const unsigned long DHT_INTERVAL = 5000; // 5 seconds in milliseconds
unsigned long lastStirrerMove = 0; // Last time stirrer moved
const unsigned long STIRRER_MOVE_INTERVAL = 100; // Stirrer movement interval (ms)
int stirrerPosition = 0; // Current stirrer position (0-180)
int stirrerDirection = 1; // Stirrer movement direction (1 or -1)
...
void setup() {
  ...
  stirrerServo.setPeriodHertz(50);
  stirrerServo.attach(STIRRER_SERVO_PIN, 500, 2400);
  stirrerServo.write(90); // Start stirrer at center position
  delay(100);
  ...
  // 5. Setup Buttons (Internal Pullup Resistor)
  // This is crucial: It keeps the pin HIGH when not pressed.
  pinMode(BUTTON_PIN, INPUT_PULLUP);
  pinMode(RELAY_BUTTON_PIN, INPUT_PULLUP);
  pinMode(STIRRER_BUTTON_PIN, INPUT_PULLUP);

  // 6. Setup Relay (DC Motor)
...
void loop() {
  ...
  // --- PART 2: BUTTON CONTROL (Lid & Yellow LED) ---
  int currentButtonState = digitalRead(BUTTON_PIN);

  // Detect the "Press" (Transition from HIGH to LOW)
  if (lastButtonState == HIGH && currentButtonState == LOW) {
    ...
    if (isLidOpen) {
      ...
      Serial.println("Action: Closing Lid...");
      closeLid();
    }
  ...
  // --- PART 3: RELAY BUTTON CONTROL (DC Motor & LED) ---
  int currentRelayButtonState = digitalRead(RELAY_BUTTON_PIN);
  ...
  if (lastRelayButtonState == HIGH && currentRelayButtonState == LOW) {
    ...
    if (isRelayOn) {
      Serial.println("Action: Turning ON Relay/Motor...");
      turnOnRelay();
    } else {
      Serial.println("Action: Turning OFF Relay/Motor...");
      turnOffRelay();
    }
  ...
  // --- PART 4: STIRRER BUTTON CONTROL (Stirrer Servo & LED) ---
  int currentStirrerButtonState = digitalRead(STIRRER_BUTTON_PIN);

  // Detect the "Press" (Transition from HIGH to LOW)
  if (lastStirrerButtonState == HIGH && currentStirrerButtonState == LOW) {
    
    // Toggle the stirrer state
    isStirrerOn = !isStirrerOn; 
    
    if (isStirrerOn) {
      Serial.println("Action: Starting Stirrer...");
      startStirrer();
    } else {
      Serial.println("Action: Stopping Stirrer...");
      stopStirrer();
    }
  ...
  lastStirrerButtonState = currentStirrerButtonState;

  // --- PART 5: STIRRER MOVEMENT (if active) ---
  if (isStirrerOn) {
    handleStirrerMovement(currentTime);
  }
}
...
void startStirrer() {
  stirrerPosition = 90; // Start at center
  stirrerDirection = 1; // Start moving forward
  stirrerServo.write(stirrerPosition);
  startActionBeep(); // Beep on stirrer state change
}

void stopStirrer() {
  stirrerServo.write(90); // Return to center position
  startActionBeep(); // Beep on stirrer state change
}

void handleStirrerMovement(unsigned long currentTime) {
  if (currentTime - lastStirrerMove >= STIRRER_MOVE_INTERVAL) {
    lastStirrerMove = currentTime;
    
    // Move stirrer back and forth (0 to 180 degrees)
    stirrerPosition += stirrerDirection * 10; // Move by 10 degrees
    
    // Reverse direction at limits
    if (stirrerPosition >= 180) {
      stirrerPosition = 180;
      stirrerDirection = -1;
    } else if (stirrerPosition <= 0) {
      stirrerPosition = 0;
      stirrerDirection = 1;
    }
...
void reconnectMQTT() {
  ...
  while (!mqttClient.connected() && mqttAttempts < 3) {
    ...
    if (mqttClient.connect(clientId.c_str())) {
      ...
      if (mqttClient.subscribe(MQTT_CMD_STIRRER)) {
        Serial.print("MQTT: Subscribed to ");
        Serial.println(MQTT_CMD_STIRRER);
      }
...
void mqttCallback(char* topic, byte* payload, unsigned int length) {
  ...
  // Handle stirrer commands
  else if (topicStr == MQTT_CMD_STIRRER) {
    if (action == "START") {
      isStirrerOn = true;
      startStirrer();
      publishStatus(MQTT_STATUS_STIRRER, "RUNNING");
      Serial.println("MQTT: Stirrer started");
    } else if (action == "STOP") {
      isStirrerOn = false;
      stopStirrer();
      publishStatus(MQTT_STATUS_STIRRER, "STOPPED");
      Serial.println("MQTT: Stirrer stopped");
    } else {
      Serial.print("MQTT: Invalid stirrer action: ");
      Serial.println(action);
    }
  }
  else {
...
void publishSensorData() {
  ...
  if (!isnan(t) && !isnan(h)) {
    ...
    // Format: {"temperature": 30.80, "humidity": 70.00, "timestamp": "2024-01-03T16:58:08Z", "lid_state": "CLOSED", "fan_state": "OFF", "stirrer_state": "OFF"}
    if (timestamp.length() > 0) {
      snprintf(mqttBuffer, sizeof(mqttBuffer), 
               "{\"temperature\": %.2f, \"humidity\": %.2f, \"timestamp\": \"%s\", \"lid_state\": \"%s\", \"fan_state\": \"%s\", \"stirrer_state\": \"%s\"}",
               t, h, timestamp.c_str(),
      ...
      snprintf(mqttBuffer, sizeof(mqttBuffer), 
               "{\"temperature\": %.2f, \"humidity\": %.2f, \"lid_state\": \"%s\", \"fan_state\": \"%s\", \"stirrer_state\": \"%s\"}",
               t, h,
               isLidOpen ? "OPEN" : "CLOSED",
               isRelayOn ? "ON" : "OFF",
               isStirrerOn ? "ON" : "OFF");
    ...
    if (timestamp.length() > 0) {
      snprintf(mqttBuffer, sizeof(mqttBuffer), 
               "{\"error\": \"DHT11 sensor error\", \"timestamp\": \"%s\", \"lid_state\": \"%s\", \"fan_state\": \"%s\", \"stirrer_state\": \"%s\"}",
               timestamp.c_str(),
               isLidOpen ? "OPEN" : "CLOSED",
               isRelayOn ? "ON" : "OFF",
               isStirrerOn ? "ON" : "OFF");
      ...
      snprintf(mqttBuffer, sizeof(mqttBuffer), 
               "{\"error\": \"DHT11 sensor error\", \"lid_state\": \"%s\", \"fan_state\": \"%s\", \"stirrer_state\": \"%s\"}",
               isLidOpen ? "OPEN" : "CLOSED",
               isRelayOn ? "ON" : "OFF",
               isStirrerOn ? "ON" : "OFF");
    }
